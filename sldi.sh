...
if [[ "${sha_used_img}" == "${sha_latest_img}" ]]; then
    ...
    echo "The latest image name: ${latest_img_name}. The name is stored in env under DIC_LATEST_IMG. Run using source to be able to see the stored environment variable"
    export DIC_LATEST_IMG="${latest_img_name}"
fi
